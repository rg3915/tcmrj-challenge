...
def gen_numbers(min_length=0, max_length=1000):
    return randint(min_length, max_length)
...
def gen_title(nb_words=6, remove_dot=False):
    title = fake.sentence(nb_words=nb_words)
    if remove_dot:
        return title[:-1]
    return title
